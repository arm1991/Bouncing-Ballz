...
import { getItemByName } from "../helpers/helpers.js";
...
class Ball {
  ...
  changePosition({ acceleration, deltaTime }) {
    ...
    let distance =
      0.5 * acceleration * deltaTime * deltaTime + this.velocityY * deltaTime;
...
class Pingpong extends Ball {
  constructor(x, y, id) {
    super(x, y, id);
    this.img = getItemByName({ name: "pingpong", items: ballImages }).img;
...
class Basketball extends Ball {
  constructor(x, y, id) {
    super(x, y, id);
    this.img = getItemByName({ name: "basketball", items: ballImages }).img;
...
class Football extends Ball {
  constructor(x, y, id) {
    super(x, y, id);
    this.img = getItemByName({ name: "football", items: ballImages }).img;
...
class Tennis extends Ball {
  constructor(x, y, id) {
    super(x, y, id);
    this.img = getItemByName({ name: "tennis", items: ballImages }).img;
...
class Bowling extends Ball {
  constructor(x, y, id) {
    super(x, y, id);
    this.img = getItemByName({ name: "bowling", items: ballImages }).img;
